...
static LIS3DSH_DMA lis;
static MPU6050_DMA mpu;
...
{
    console=uart;
    MPU6050_DMA_Init(&mpu,i2c);
    LIS3DSH_DMA_Init(&lis,spi);
...
{
    LIS3DSH_DMA_Process(&lis);
    MPU6050_DMA_Process(&mpu);
    ...
    if (uart_fault || lis.io.state==SDMA_FAULT || mpu.io.state==SDMA_FAULT)
    ...
    int length=snprintf(uart_buffer,sizeof(uart_buffer),
        ...
        SensorDMA_StateName(lis.io.state),(unsigned)lis.io.last_hal,(unsigned long)lis.io.hal_error,
        ...
        (unsigned long)mpu.io.max_transfer_ms,SensorDMA_StateName(mpu.io.state),
...
{
    if (pin==GPIO_PIN_0) SensorDMA_DataReady(&lis.io);
    if (pin==GPIO_PIN_2) SensorDMA_DataReady(&mpu.io);
}
void HAL_SPI_TxRxCpltCallback(SPI_HandleTypeDef *spi)
{ if (spi==lis.base.spi) LIS3DSH_DMA_Complete(&lis); }
void HAL_SPI_ErrorCallback(SPI_HandleTypeDef *spi)
{ if (spi==lis.base.spi) LIS3DSH_DMA_Error(&lis); }
void HAL_I2C_MemRxCpltCallback(I2C_HandleTypeDef *i2c)
{ if (i2c==mpu.base.i2c) MPU6050_DMA_Complete(&mpu); }
void HAL_I2C_ErrorCallback(I2C_HandleTypeDef *i2c)
{ if (i2c==mpu.base.i2c) MPU6050_DMA_Error(&mpu); }
